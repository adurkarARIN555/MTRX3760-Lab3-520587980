...
// Function to add a gate to the circuit with the specified type and name
void BuildCircuit::addGate(const std::string& type, const std::string& name)
{
    // Conditional statements to make sure that the circuit is connected/made-of to the specified gate
    if (type == "and")
    {
...
void BuildCircuit::connect(const std::string& sourceName, const std::string& targetName, int inputIndex)
{
    ...
    {
        eLogicLevel sourceOutput = gates[sourceName]->getOutput();      // Dereferencing the pointer (pass-by-reference)
    ...
    {
        eLogicLevel wireLevel = wires[sourceName]->getLevel();          // Dereferencing the pointer (pass-by-reference)
        if (gates.find(targetName) != gates.end())
        {
...
eLogicLevel BuildCircuit::getOutput(const std::string& gateName)
{
    ...
    {
        gates[gateName]->ComputeOutput();                               // Dereferencing the pointer (pass-by-reference)
...
void BuildCircuit::setInput(const std::string& wireName, eLogicLevel level)
{
    ...
    {
        wires[wireName]->setLevel(level);                               // Dereferencing the pointer (pass-by-reference)
    }
}
